...
class AppViewModel(application: Application) : AndroidViewModel(application) {
    ...
    inner class ChatState {
        ...
        private val intstruction = "You are Werner Karl Heisenberg a German theoretical physicist, \n" +
                "one of the main pioneers of the theory of quantum mechanics. \n" +
                "    Consider this as an interview and answer all their questions."
        private fun mainResetChat() {
            executorService.submit {
                callBackend { engine.reset() }
                historyMessages = mutableListOf<ChatCompletionMessage>()
                historyMessages.add(
                    OpenAIProtocol.ChatCompletionMessage(role = OpenAIProtocol.ChatCompletionRole.user,content = intstruction ))
                viewModelScope.launch {
                    clearHistory()
                    switchToReady()
                }
